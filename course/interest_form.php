...
/**
 * Application form - Expressions of Interest in Future Courses
 */


global $list_for_module_select;

$list_for_module_select[''] = 'Select...';
$i = 1;
$list_for_module_select[$i++] = 'Biostatistics';
$list_for_module_select[$i++] = 'Communicable Disease';
$list_for_module_select[$i++] = 'Disaster Management and Emergency Planning';
$list_for_module_select[$i++] = 'Evaluation of Interventions';
$list_for_module_select[$i++] = 'Evidence Based Practice';
$list_for_module_select[$i++] = 'Health Economics';
$list_for_module_select[$i++] = 'Health Promotion';
$list_for_module_select[$i++] = 'HIV/AIDS';
$list_for_module_select[$i++] = 'Inequalities and Social Determinants of Health';
$list_for_module_select[$i++] = 'Introduction to Epidemiology';
...
$list_for_module_select[$i++] = 'Conceptos de Salud Pública para la Elaboración de Políticas [Español]';
$list_for_module_select[$i++] = 'Concepts de Santé Publique pour décideurs [Français]';
...
$editform = new interest_form_form(NULL, array('customdata' => array()));
...
elseif ($data = $editform->get_data()) {
  ...
  $interest->firstname = $dataitem;

  $dataitem = $data->lastname;
  if (empty($dataitem)) $dataitem = '';
  $dataitem = trim(strip_tags($dataitem));
  $dataitem = mb_substr($dataitem, 0, 100, 'UTF-8');
  $interest->lastname = $dataitem;

  $dataitem = $data->email;
  if (empty($dataitem)) $dataitem = '';
  $dataitem = trim(strip_tags($dataitem));
  $dataitem = mb_substr($dataitem, 0, 100, 'UTF-8');
  $interest->email = $dataitem;

  $dataitem = $data->country;
  if (empty($dataitem)) $dataitem = '';
  $interest->country = $dataitem;

  $dataitem = $data->course_id_1;
  if (empty($dataitem)) $dataitem = '';
  if ($dataitem === '' || (is_numeric($dataitem) && $dataitem == 0)) {
    $interest->coursename1 = '';
  }
  else {
    $interest->coursename1 = $list_for_module_select[$dataitem];
  }

  $dataitem = $data->course_id_2;
  if (empty($dataitem)) $dataitem = '';
  if ($dataitem === '' || (is_numeric($dataitem) && $dataitem == 0)) {
    $interest->coursename2 = '';
  }
  else {
    $interest->coursename2 = $list_for_module_select[$dataitem];
  }

  $dataitem = $data->suggestions;
  if (empty($dataitem)) $dataitem = '';
  $interest->suggestions = htmlspecialchars($dataitem, ENT_COMPAT, 'UTF-8');
  ...
  sendapprovedmail('techsupport@peoples-uni.org', "Peoples-uni Expression of Interest in Future Courses From: $interest->lastname, $interest->firstname", $message);

  redirect(new moodle_url($CFG->wwwroot . '/course/interest_form_success.php'));
}


// Print the form

$PAGE->set_title("People's Open Access Education Initiative Expressions of Interest in Future Courses");
$PAGE->set_heading('Peoples-uni Application Form - Expressions of Interest in Future Courses');
