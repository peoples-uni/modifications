<?php  // $Id: once_off_import_interest_to_moodle.php,v 1.1 2011/04/14 14:23:00 alanbarrett Exp $
/**
*
* On Once Off Basis ONLY!... Import Drupal Webform Interest Submisisons to Moodle Database
*
*/
...
// comment:			  9
  ...
  $interest->country = $registration['8'];

  if (empty($registration['10'])) $registration['10'] = '0';
  if (empty($registration['10'])) $rowdata[] = '';
  else $rowdata[] = 'Yes';
  $interest->state = $registration['10'];

  if (empty($registration['9'])) $registration['9'] = '';
  $interest->comment = htmlspecialchars($registration['9'], ENT_COMPAT, 'UTF-8');
  $rowdata[] = str_replace("\r", '', str_replace("\n", '<br />', $interest->comment));

  if (substr($registration['9'], 0, 6) === 'HIDDEN') $interest->hidden = 1;
