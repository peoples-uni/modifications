...
ALTER TABLE mdl_peoplesmph2 ADD willpay   BIGINT(10) UNSIGNED NOT NULL DEFAULT 0 AFTER entitled;
...
CREATE TABLE mdl_peoples_decision (
  ...
  note_mph            TEXT NOT NULL,
  note_scholarship    TEXT NOT NULL,
CONSTRAINT PRIMARY KEY (id),
...
foreach ($applications as $sid => $application) {
  ...
  if (!$displaystudenthistory) {
    ...
    if ($application->take_final_assignment == 2) {
      if (empty($moduleCPD_only[$application->coursename1])) {
        $moduleCPD_only[$application->coursename1] = 1;
      }
      else {
        $moduleCPD_only[$application->coursename1]++;
      }
    ...
    if ($application->take_final_assignment == 3) {
      if (empty($modulestudy_materials_only[$application->coursename1])) {
        $modulestudy_materials_only[$application->coursename1] = 1;
      }
      else {
        $modulestudy_materials_only[$application->coursename1]++;
      }
